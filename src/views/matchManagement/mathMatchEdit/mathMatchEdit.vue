    ...
    <div class="cont-margin">
      <el-form ref="postForm" :model="postForm" :rules="rules" label-width="120px">
        ...
        <el-form-item prop="banner" label="广告图">
          <BannerUpload v-model="postForm.banner" @input="getInputFile" />
        </el-form-item>
        <el-form-item prop="shareBanner" label="分享页顶图">
          <ShareBannerUpload v-model="postForm.shareBanner" @input="getInputShareFile" />
        </el-form-item>
      ...
      <div class="dialog-footer">
        <el-button type="primary" size="mini" :disabled="opaList.indexOf('edit') === -1 ? true : false" @click="createClassData">
          确定
        </el-button>
      </div>
    </div>
...
import ShareUpload from '@/components/Upload/singleImage4'
import { common } from '../../../utils'
import { opaMixin } from '../../../utils/mixins'

export default {
  name: 'MathMatchEdit',
  components: { Tinymce, 'BannerUpload': Upload, 'ShareBannerUpload': ShareUpload },
  filters: {},
  mixins: [opaMixin],
  data () {
    return {
